...
function RoomSelector({ rooms, myRoom, players, roomMap, onSelect }) {
return (
<div className="rooms-grid">
{rooms.map(room => {
const here = players.filter(p => roomMap[p.id] === room.id);
return (
<div key={room.id} className={`room-card ${myRoom === room.id ? "mine" : ""}`} onClick={() => onSelect(room.id)}>
<div className="room-icon">{room.icon}</div>
<div className="room-name">{room.name}</div>
<div className="room-pips">{here.length > 0 ? here.map(p => p.emoji).join(" ") : "·"}</div>
</div>
);
})}
</div>
